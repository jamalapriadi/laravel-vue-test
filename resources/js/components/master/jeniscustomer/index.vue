    ...
    <div class="card card-accent-primary">
        <div class="card-header">
            ...
            <div class="card-header-actions">
                <router-link to="/add-jeniscustomer" class="btn btn-primary">
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    <tr v-for="(l, index) in list.data" v-bind:key="index">
                        <td>{{index+1}}</td>
                        <td>{{l.jns_customer}}</td>
                        <td>
                            <div class="btn-group">
                                <router-link :to="{ name: 'jenisCostumerEdit', params: {id: l.jns_customer}}" class="btn btn-warning">
                                    <i class="fa fa-edit text-white"></i>
                                </router-link>

                                <a class="btn btn-danger" v-on:click="hapus(l.jns_customer, index, l.nm)" v-bind:id="'delete'+l.kd_bank">
    ...
    export default {
        ...
        methods: {
            ...
            showData(page){
                ...
                axios.get('data/jenis-customer?page='+page)
                    .then(response => {
                        this.loading=false;
                        this.list = response.data;
            ...
            cariData(page){
                ...
                axios.get('/data/jenis-customer?q='+this.pencarian)
                    .then(response => {
                        this.list = response.data;
                    })
                    .catch( errors => {
            ...
            hapus(id,index,name){
                ...
                .then((result) => {
                    if(result.value) {
                        axios.delete('/data/jenis-customer/'+id)
                            .then(response => {
